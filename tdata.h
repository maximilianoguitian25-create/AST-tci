...
tdata pro_cate(tdata a, tdata b){// revisar limpieza revisar y hacer leo
...
tdata list_to_str(tdata lista){
	// if si es str y si no es llamar a str de ese campo
...
void free_tdata(tdata n){
	...
	if(n->nodeType == STR) {
		...
		// Solo liberar data y next si no son NULL
